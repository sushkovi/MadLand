...
function bars() {
  ...
  bar.className = "bar";
  var os = Math.floor(ll.width * 0.66);
  ...
  setTimeout(function () {
    document.querySelector("#barBar").remove(document.querySelector("#barBar"));
  }, 1925);
}

function turnCanyon() {
  var num = Math.floor(Math.random() * 45);
  var d = Math.random() < 0.5 ? -num : num;
...
window.addEventListener("mousemove", function (e) {
  ...
  ship.style.transform =
    "translateX(" + rx * 200 + "%) rotateZ(" + rx * 10 + "deg)";
});
  ...
  btn[i].addEventListener("click", function (e) {
    ...
    function hitBars() {
      ...
      if (document.querySelector("#barBar")) {
        ...
        var ease = Math.floor(s.width * 0.25);
        if (
          (s.top < b.bottom && s.right - ease > b.right) ||
          (s.top < b.bottom && s.left + ease < b.left)
        ) {
          ship.classList.add("dead");
          setTimeout(function () {
            ship.classList.remove("dead");
      ...
      if (document.querySelector("#can")) {
        ...
        if (
          ...
          setTimeout(function () {
            ship.classList.remove("dead");
            document
              .querySelector("#canyonBox")
              .remove(document.querySelector("#canyonBox"));
          }, 2000);
          clearInterval(play);
          clearInterval(dead);
          clearInterval(can);
